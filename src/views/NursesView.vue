...
const editingNurse = ref<INurse | null>(null);
const form = ref({
  id: 0,
  name: '',
...
const openEditDialog = (nurse: INurse) => {
      ...
      <TableBody>
        <TableRow v-for="nurse in nurses" :key="nurse.id" class="hover:bg-muted">
          ...
          <TableCell class="flex justify-end">
            <Button variant="outline" size="sm" @click="openEditDialog(nurse)"
              >Редактировать</Button
            >
            <Button variant="destructive" size="sm" class="ml-2" @click="deleteNurse(nurse.id)"
              >Удалить</Button
            >
          </TableCell>
        </TableRow>
